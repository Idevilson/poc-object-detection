...
FrameSampler::FrameSampler(
  ...
  for (size_t planeIndex = 0; planeIndex < framePlanes.size(); ++planeIndex) {
    ...
    // branch. readPlane is only ever asked for pixels inside
    // [0, sampleWidth - 1] x [0, sampleHeight - 1], and NV12 color planes
    // additionally read componentOffset 1, so this is the largest byte offset
    // it can ever produce for this plane.
